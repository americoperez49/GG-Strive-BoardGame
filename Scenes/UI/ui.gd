...
func _ready():
	...
	Client.player_has_disconnected_from_server.connect(_on_player_has_disconnected_from_server)


func _create_ready_list():
	_clear_player_ready_list()
...
func _disable_ready_button():
	print("Ready button disabled on client: " + str(multiplayer.get_unique_id()))
	ready_button.disabled = true
	join_button.show()
	
func _clear_player_ready_list():
...
func _on_player_has_disconnected_from_server():
	_create_ready_list()	
	_disable_ready_button()
